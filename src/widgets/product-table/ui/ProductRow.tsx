...
import { formatRating } from '@shared/lib/utils/formatters';
import { PriceInRubles } from '@features/currency-converter';
...
export const ProductRow = ({
  ...
  return (
    <tr className={`${styles.row} ${isSelected ? styles.selected : ''}`}>
      <td className={styles.cell} data-column="select">
        <Checkbox 
          checked={isSelected}
          onChange={() => onSelect(product.id)}
        />
      </td>
      <td className={styles.cell} data-column="title">
      ...
      <td className={styles.cell} data-column="brand">
        <span className={styles.vendor}>{product.brand}</span>
      </td>
      <td className={styles.cell} data-column="sku">
        <span className={styles.sku}>{product.sku}</span>
      </td>
      <td className={styles.cell} data-column="rating">
        <span className={styles.rating}>
          {isLowRating ? (
            <>
              <span className={styles.lowRating}>{product.rating.toFixed(1)}</span>
              <span>/5</span>
            </>
          ) : (
      ...
      <td className={styles.cell} data-column="price">
        <span className={styles.price}>
          <PriceInRubles price={product.price} currency="USD" />
        </span>
      </td>
